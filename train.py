...
class Trainer:
    def __init__(self, config:Config, device:torch.device, mode:str, continuous:int):
        ...
        if self.config.img_size < 8:
            print('height and width you expected have to larger than 8')
        ...
        if self.config.CelebA_train:
            ...
            if not os.path.isdir(self.base_path + 'CelebA'):
                try:
                    print('Downloading...')
                    wget_path = self.base_path + 'CelebA.zip'
                    unzip_path = self.base_path + 'CelebA'
                    ...
                    print('Command not found. Please check mkdir, wget, unzip, rm command')
                    raise e
            ...
            trans = transforms.Compose([transforms.Grayscale(num_output_channels=1),
                                ...
                                transforms.ToTensor(),
                                transforms.Normalize((0.5), (0.5))]) if self.convert2grayscale else \
                    transforms.Compose([transforms.Resize(self.config.img_size),
                                transforms.CenterCrop(self.config.img_size),
                                transforms.ToTensor(),
                                transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
            dataset = dsets.ImageFolder(root=self.base_path+'CelebA', transform=trans)
            self.trainset, self.valset, self.testset = random_split(dataset, [len(dataset)-2000, 1000, 1000])
        ...
        self.fixed_test_noise = torch.randn(128, self.noise_init_size, 1, 1).to(device)
        ...
        if self.mode == 'train':
            self.G_optimizer = optim.Adam(self.G_model.parameters(), lr=self.lr, betas=(self.beta1, 0.999))
            self.D_optimizer = optim.Adam(self.D_model.parameters(), lr=self.lr, betas=(self.beta1, 0.999))
            if self.continuous:
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for phase in ['train', 'val']:
                ...
                for i, (real_data, _) in enumerate(self.dataloaders[phase]):
                    ...
                    with torch.set_grad_enabled(phase=='train'):
                        ...
                        fake_data = self.G_model(torch.randn(batch_size, self.noise_init_size, 1, 1).to(self.device))
            ...
            fake_imgs = self.G_model(self.fixed_test_noise)
        ...
        return {'discriminator': self.D_model, 'generator': self.G_model}, self.loss_data
